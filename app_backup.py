...
from house_price_prediction.src.data import load_data
...
with tab_home:
    ...
    with p3:
        with st.container(border=True):
            ...
            st.write("""
            ML Model trained to predict the house price. 
            With ElasticsNet Linear Regression
            """)
            st.write("**R² = 83%** · **MAPE = 2.22%**")
            st.info("👉 Try it live in the **🤖 ML Predictor** tab above or **📈 EDA** tab above.")
...
def show_house_price_pred():
    ...
    st.write(
        "This project predicts House Price "
        "based on another model"
    )
    try:
        artifact = get_artifact()
    except FileNotFoundError:
